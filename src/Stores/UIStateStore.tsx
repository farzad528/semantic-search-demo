...
const initialToggleState = isNaN(initialToggleStateNum)
  ? 0
  : initialToggleStateNum;
const initialToggleStateBools = [0, 1, 2, 3, 4]
  .map((num) => initialToggleState & (1 << num))
  .map((state) => !!state);
...
export default ({ children }: { children: any }) => {
  const [
    ...
    {
      toggle: toggleIsCompareToDefaultOn,
      setFalse: setIsCompareToDefaultOff,
      setTrue: setIsCompareToDefaultOn,
    },
  ] = useBoolean(initialToggleStateBools[0]);
  const [
    isSpellingOn,
    {
      toggle: toggleIsSpellingOn,
      setFalse: setIsSpellingOnOff,
      setTrue: setIsSpellingOnOn,
    },
  ] = useBoolean(initialToggleStateBools[1]);
  const [
    isRankOn,
    {
      toggle: toggleIsRankOn,
      setFalse: setIsRankOnOff,
      setTrue: setIsRankOnOn,
    },
  ] = useBoolean(initialToggleStateBools[2]);
  const [
    isAnswersOn,
    {
      toggle: toggleIsAnswersOn,
      setFalse: setIsAnswersOnOff,
      setTrue: setIsAnswersOnOn,
    },
  ] = useBoolean(initialToggleStateBools[3]);
  const [
    isCaptionsOn,
    {
      toggle: toggleIsCaptionsOn,
      setFalse: setIsCaptionsOnOff,
      setTrue: setIsCaptionsOnOn,
    },
  ] = useBoolean(initialToggleStateBools[4]);

  const [isTeachingBubbleVisible, { toggle: toggleTeachingBubbleVisible }] =
    useBoolean(false);
  ...
  const [pagination, setPagination] = useState<number>(
    () => Number(queryParams.get("pagination")) || 0
  );
  const [datasetKind, setDatasetKind] = useState<DatasetKind>(
    () => Number(queryParams.get("dataset")) || DatasetStore.getDefaultDataset()
  );

  const [defaultSearchResponse, setDefaultSearchResponse] =
    useState<SearchResponse>();
  const [semanticSearchResponse, setSemanticSearchResponse] =
    useState<SearchResponse>();
  ...
  useEffect(() => {
    if (isOnSearchPage) {
      history.replace(
        `/search?query=${encodeUrlComponent(
          query
        )}&dataset=${datasetKind}&pagination=${pagination}&toggleState=${getToggleState()}`
      );
    }
  ...
  const setDatasetKindCallback: Dispatch<DatasetKind> = (
    newDatasetKind: DatasetKind
  ) => {
  ...
  const store: UIStateStore = {
    isCompareToDefaultOn: [
      isCompareToDefaultOn,
      toggleIsCompareToDefaultOnCallback,
    ],
  ...
  return (
    <UIStateStoreContext.Provider value={store}>
      {" "}
      {children}{" "}
