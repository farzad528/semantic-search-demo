...
export const SearchResultContainer: React.FunctionComponent<
  SearchResultContainerProps
> = ({
  isCompareToDefaultOn,
  defaultDocuments,
  answer,
  shouldUseDefaultDocuments,
  documents,
  query,
}) => {
    ...
    <Stack horizontal gap={128} styles={styles.searchResultsContainer}>
          ...
          {defaultDocuments?.length === 0 ? (
            ...
            defaultDocuments?.map((document, index) => (
              <DocumentResultCard
                key={document.id}
                document={document}
                isSingleColumn={!isCompareToDefaultOn}
              />
            ))
          )}
        </Stack>
      ...
      <Stack gap={22} style={{ minWidth: isCompareToDefaultOn ? "auto" : 960 }}>
        ...
        <FlipMove
          ...
          leaveAnimation={"none"}
        >
          {(shouldUseDefaultDocuments ? defaultDocuments : documents)
            ?.length === 0 ? (
            <p style={styles.noResults}>No results</p>
          ) : (
            (shouldUseDefaultDocuments ? defaultDocuments : documents)?.map(
              (document, index) => (
                <DocumentResultCard
                  key={document.id}
                  document={document}
