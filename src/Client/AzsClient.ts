...
const getRequestCacheKey = (body: object, indexName: string) =>
  md5(JSON.stringify(body, Object.keys(body).sort()) + indexName).toString();

// THE ENDPOINT NEEDS TO ENABLE CORS.
const getDemo1Endpoint = (indexName: string) =>
  `${process.env.REACT_APP_SEARCH_DOMAIN}/indexes/${indexName}/docs/search?api-version=2021-04-30-Preview`;
const getDemo2Endpoint = (indexName: string) =>
  `${process.env.REACT_APP_SEARCH_DOMAIN}/indexes/${indexName}/docs/search?api-version=2021-04-30-Preview`;
...
async function fetchData(
  ...
  const fetchRetry: (...args: any[]) => Promise<Response> =
  ...
  let response = await fetchRetry(
    isRetry ? getDemo2Endpoint(indexName) : getDemo1Endpoint(indexName),
    {
      method: "POST",
      cache: "no-cache",
      headers: {
